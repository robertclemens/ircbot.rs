...
use crate::consts::*;
use crate::cstr::{eq_ic, now, starts_with_ic, trunc, trunc_string, Tok};
use crate::state::{lww_next_ts, BotState, ChanReq, ChanStatus, RosterEntry, M_I, M_K, S_AUTHED};
...
fn handle_mode_change(state: &mut BotState, channel: &str, modes: &str, args: &str) {
    let Some(ci) = channel::find(state, channel) else { return };
    let mut adding = true;
                ...
                if mode == 'o' {
                    if eq_ic(arg, &state.current_nick) {
                        ...
                        if adding {
                            ...
                            logm!(state, L_INFO, "[INFO] I was DEOPPED in {}. Refreshing roster to find a trusted op.\n", channel);
...
fn update_own_hostmask(state: &mut BotState, mask: &str) {
    ...
    logm!(state, L_INFO, "[INFO] My hostmask is now: {}\n", state.actual_hostname);
    ...
    match command {
        ...
        "396" => {
            if let Some(host) = second_tok(params) {
                ...
                logm!(state, L_INFO, "[INFO] Displayed host changed to {}; re-checking my mask\n", host);
        ...
        "PART" => {
            if let Some(p) = prefix {
                ...
                if nick.is_some_and(|n| eq_ic(n, &state.current_nick) || eq_ic(n, &state.target_nick)) {
...
fn on_welcome(state: &mut BotState, params: &str) {
    ...
    if !srv_nick.is_empty() && srv_nick.len() < MAX_NICK && state.current_nick != srv_nick {
        logm!(
            ...
            srv_nick,
            if state.current_nick.is_empty() { "unset" } else { state.current_nick.as_str() }
        );
...
fn on_lockout(state: &mut BotState, params: &str, kind: ChanReq) {
    ...
    let Some(ci) = channel::find(state, ch) else { return };
    state.chans[ci].status = ChanStatus::Out;
    if state.chans[ci].is_managed {
        match kind {
            ChanReq::Unban => logm!(state, L_INFO, "[474] Banned from {}, requesting unban\n", ch),
            _ => logm!(state, L_INFO, "[475] Bad key for {}, requesting current key\n", ch),
        }
...
fn on_mode(state: &mut BotState, params: &str) {
    ...
    let (Some(target), Some(modes)) = (target, modes) else { return };
...
fn on_who_reply(state: &mut BotState, params: &str) {
    ...
    let nick = t.next(" ");
    let modes = t.next(" ");
    let (Some(chan_name), Some(ident), Some(host), Some(nick), Some(modes)) = (chan_name, ident, host, nick, modes) else {
        return;
    };
    ...
    let Some(ci) = channel::find(state, chan_name) else { return };
    let c = &mut state.chans[ci];
...
fn on_end_of_who(state: &mut BotState, params: &str) {
    ...
    logm!(state, L_INFO, "[INFO] Roster for {} updated with {} users.\n", name, roster_len);
    ...
    logm!(state, L_DEBUG, "[OP-REQ] trusted_bot_count={}\n", state.trusted_bots.len());
    for (i, tb) in state.trusted_bots.iter().enumerate() {
        logm!(state, L_DEBUG, "[OP-REQ] trusted_bots[{}]: {}|{}{}\n", i, tb.mask, tb.uuid, if tb.has_pub { "" } else { " (no key)" });
    }
    ...
    if c.op_request_retry_count >= 5 && now - c.last_op_request_time > 300 {
        logm!(state, L_INFO, "[INFO] Resetting retry counter for {} after timeout.\n", name);
        state.chans[ci].op_request_retry_count = 0;
    }
    if state.chans[ci].op_request_retry_count >= 5 {
        logm!(state, L_INFO, "[INFO] Gave up requesting ops in {} after {} attempts.\n", name, state.chans[ci].op_request_retry_count);
        return;
    }

    logm!(state, L_DEBUG, "[OP-REQ] Scanning {} roster entries for trusted ops\n", roster_len);
    ...
    if now - state.last_op_request_sent >= OP_REQUEST_MIN_INTERVAL {
        ...
        logm!(state, L_DEBUG, "[OP-REQ] Looking up UUID for helper: nick={} hostmask={}\n", chosen.nick, chosen.hostmask);
        if let Some(uuid) = find_uuid_for_helper(state, &chosen) {
            ...
            logm!(state, L_DEBUG, "[OP-REQ] hub_client_request_op returned: {}\n", sent_via_hub as i32);
        }
        if !sent_via_hub {
            logm!(state, L_DEBUG, "[OP-REQ] Hub unavailable or no UUID match, falling back to PRIVMSG\n");
            bot_comms::send_to_host(state, &chosen.hostmask, &chosen.nick, &format!("OPME {name}"));
        }
...
fn on_privmsg(state: &mut BotState, prefix: &str, params: &str) {
    ...
    let (Some(nick), Some(dest), Some(message)) = (nick, dest, message) else { return };
...
fn on_join(state: &mut BotState, prefix: &str, params: &str) {
    ...
    if join_mask_ok && eq_ic(nick, &state.current_nick) && prefix.contains('!') && prefix.contains('@') {
    ...
    if !state.chans[ci].is_managed {
        ...
        let name = state.chans[ci].name.clone();
        logm!(state, L_INFO, "[INFO] {} was removed while joining; parting\n", name);

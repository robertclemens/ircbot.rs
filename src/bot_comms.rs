...
use crate::consts::*;
use crate::cstr::{Tok, eq_ic, now};
use crate::state::{BotState, ChanReq, ChanStatus, has_control_bytes, is_rfc_nick};
...
fn open_and_dispatch(state: &mut BotState, ti: usize, b64: &str, sender_nick: Option<&str>) {
    ...
    if !sender.has_pub || sender.uuid.is_empty() {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] ~B2 from {} dropped: no public key on file yet\n",
            sender.mask
        );
        return;
    }
    ...
    if b64.len() > A2_B64_MAX {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] ~B2 from {} oversized; dropped\n",
            sender.mask
        );
        return;
    }
    let frame = match crypto::b64_decode(b64) {
        ...
        _ => {
            logm!(
                state,
                L_CMD,
                "[BOT-COMM] ~B2 from {} malformed; dropped\n",
                sender.mask
            );
            return;
        }
    ...
    let Some(pt) = pt else {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] ~B2 from {} did not open; dropped\n",
            sender.mask
        );
        return;
    };
    if has_control_bytes(&pt) {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] Control character in command from {}; dropped\n",
            sender.mask
        );
        return;
    }
    ...
    if (now - ts).abs() > B2_TS_SKEW {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] ~B2 from {}: stale timestamp\n",
            sender.mask
        );
        return;
    }
    if state.recent_nonces.seen(nonce, now) {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] ~B2 replay from {}; dropped\n",
            sender.mask
        );
        return;
    }
    ...
    let (Some(verb), Some(arg1)) = (t.next(" "), t.next(" ")) else {
        return;
    };
    if eq_ic(verb, "OPME") {
        ...
        if is_rfc_nick(arg1) {
            ...
            logm!(
                state,
                L_INFO,
                "[BOT-COMM] SETNICK from {} refused: '{}' is not a valid IRC nick\n",
                sender.uuid,
                arg1
            );
        }
...
pub fn process_payload(state: &mut BotState, payload: &str) {
    ...
    let Some(b64) = payload[bar + 1..].strip_prefix("~B2 ") else {
        logm!(
            state,
            L_CMD,
            "[BOT-COMM] Relayed frame from {} is not ~B2 (pre-passwordless sender?); dropped\n",
            uuid
        );
        return;
    };
    match auth::trusted_bot_by_uuid(state, uuid) {
        ...
        None => logm!(
            state,
            L_CMD,
            "[BOT-COMM] Relayed ~B2 from unknown bot {}\n",
            uuid
        ),
    }
}
...
pub fn handle_privmsg(state: &mut BotState, nick: &str, user_host: &str, message: &str) -> bool {
    let Some(b64) = message.strip_prefix("~B2 ") else {
        return false;
    };
    match auth::trusted_bot_by_host(state, user_host) {
        ...
        None => logm!(
            state,
            L_CMD,
            "[BOT-COMM] ~B2 from untrusted {} dropped\n",
            user_host
        ),
    }
    true
...
fn b2_send(state: &mut BotState, ti: Option<usize>, target_nick: &str, command: &str) {
    let tb = match ti.map(|i| state.trusted_bots[i].clone()) {
        ...
        _ => {
            logm!(
                state,
                L_DEBUG,
                "[BOT-COMM] Cannot send to {}: no public key on file\n",
                target_nick
            );
            return;
        }
    ...
    let frame = hub_client::bot_key_decode(state).and_then(|(_ed, x)| {
        crypto::seal(Some((&x, &self_x)), &peer_x, B2_LABEL, &aad, pt.as_bytes())
    });
    drop(pt);
    ...
    if !sent_via_hub {
        logm!(
            state,
            L_DEBUG,
            "[BOT-COMM] Sending ~B2 PRIVMSG to {}: {}\n",
            target_nick,
            command
        );
        ircf!(state, "PRIVMSG {} :{}\r\n", target_nick, line);
...
mod tests {
    ...
    fn envelope_shapes() {
        assert_eq!(
            envelope_parse("123:0123456789abcdef:OPME #x"),
            Some((123, 0x0123456789abcdef, "OPME #x"))
        );

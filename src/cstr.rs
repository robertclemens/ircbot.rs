...
//!
//! The bot exchanges '|'-delimited records and IRC lines with C peers
//! (irchub, older bots).  Their parsers are strtok_r / sscanf / strtoll, so
//! the helpers here reproduce those rules exactly: which tokens a strtok_r
//! walk yields, how many conversions an sscanf field-width format makes, and
//! where snprintf into a fixed buffer cuts a string.
...
/// "8-4-4-4-12" hex UUID shape (config.c is_uuid_field).
...
/// Dashes in the UUID positions only (the looser check the hub-connect and
/// `m|` paths use).
...
mod tests {
    ...
    fn strtok_matches_c() {
        ...
        let mut t = Tok::new("nick!user@host");
        assert_eq!(t.next("!"), Some("nick"));
        assert_eq!(t.next("@"), Some("user"));
        assert_eq!(t.rest(), Some("host"));
        let mut t = Tok::new("server.only");
        assert_eq!(t.next("!"), Some("server.only"));
        assert_eq!(t.next("@"), None);
    }
